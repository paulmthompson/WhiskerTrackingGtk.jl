...
function follicle_location_cb(w::Ptr,user_data::Tuple{Tracker_Handles})
    ...
    if get_gtk_property(han.b["follicle_location_toggle"],:active,Bool)
        han.selection_mode = 17
    else
        han.selection_mode = 1
    end
...
function select_follicle_location(han::Tracker_Handles,m_x,m_y)

    i = han.displayed_frame
    while (han.man.contact[i] == 2)
        han.tracked_w.follicle_x[i] = m_x
        han.tracked_w.follicle_y[i] = m_y
        i += 1
    end
...
function save_tracked_whisker(han::Tracker_Handles,file_path)
    file = jldopen(file_path,"w")
        ...
        write(file,"parabola_coeffs",han.tracked_w.parabola_coeffs)
        write(file,"parabola_angle",han.tracked_w.parabola_angle)
